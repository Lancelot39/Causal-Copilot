...
def user_feedback_preprocess(message: str, file_path: str):
    global args, global_state
    client = OpenAI(organization=args.organization, project=args.project, api_key=args.apikey)

    class Validating_Response(BaseModel):
        changes: dict 
    ...
    prompt = f"""
    Please check if the inputted message is correct.
    If it isn't, return 'Invalid'. 
    If the data is correct, return a dictionary that indicates which variables need to be updated in file. 
    The keys in the dictionary should be the variable names, and the values should be the new values. 
    If no changes are required, return an empty dictionary.
    ...
    completion = client.beta.chat.completions.parse(
        ...
        response_format=Validating_Response, 
    )

    changes = completion.choices[0].message.parsed.changes

    if isinstance(changes, str) and changes == 'Invalid':
        return 'Invalid'

    updated_file = file_content
    for variable, value in changes.items():
        updated_file = updated_file.replace(variable, value)
            
    return changes


    
def user_algorithm_selection(message: str):
    valid_algo = ['PC', 'FCI', 'GES', 'NOTEARS', 'DirectLiNGAM', 'ICALiNGAM']

    normal_message = message.strip().lower()
    for alg in valid_algo:
        if normal_message == alg.lower():
            return alg
...
def process_message(message, chat_history, download_btn):
    ...
    if message == 'Interactive Mode':
        ...
        chat_history.append((message, 'Interactive Mode is set, please input a data set!'))
        yield chat_history, download_btn
    else:
    ...
    try:
        ...
        if REQUIRED_INFO["current_stage"] == 'ts_check_done':
         ...
         # comment0: user feedback should be processed before entering the eda_generation block
            if interactive_mode:
                ...
                chat_history.append('Here we just finished precessing the data! Please enter anything you want to add! Type Quit to exit Interactive Mode', 
                    None)
                yield chat_history, download_btn
                if message == 'Quit':
                    interactive_mode = False
            ...
            # comment3: if don't need user feedback, directly update REQUIRED_INFO["current_stage"] and enter eda_generation block
                ...
                REQUIRED_INFO["current_stage"] = 'background_knowledge'
            
            
            # comment4: add this block to process the user feedback
            if REQUIRED_INFO["current_stage"] == 'check_user_feedback':
                with open('global_state', 'w') as file:
                    file.write(str(global_state))
                if(user_feedback_preprocess(message, 'global_state') == 'Invalid'):
                    chat_history.append("That information may not be correct, please enter new data here!", None)
                    return chat_history, download_btn
                else:
                    global_state = user_feedback_preprocess(message, 'global_state')
                    REQUIRED_INFO["current_stage"] = 'background_knowledge'
        ...
        if REQUIRED_INFO["current_stage"] == 'background_knowledge':
            # Knowledge generation
            if args.data_mode == 'real':
                chat_history.append(("🌍 Generate background knowledge based on the dataset you provided...", None))
                yield chat_history, download_btn
            ...
            if interactive_mode:
                chat_history.append('Here is the background knowledge! If you have some more information, please enter it here! Press enter to continue! Type Quit to exit Interactive Mode', 
                    None)
                yield chat_history, download_btn
                if message == 'Quit':
                    interactive_mode = False
            ...
            if REQUIRED_INFO["current_stage"] == 'check_user_background':
                with open('global_state_back', 'w') as file:
                    file.write(str(global_state))
                if(user_feedback_preprocess(message, 'global_state_back') == 'Invalid'):
                    chat_history.append('That information may not be correct, please enter new data here!', None)
                    return chat_history, download_btn
                else:
                    gloabl_state = user_feedback_preprocess(message, 'global_state_back')
                    knowledge_new = str(global_state.user_data.knowledge_docs).replace("[", "").replace("]", "").replace('"',
                                                                                                                    "").replace(
                    "\\n\\n", "\n\n").replace("\\n", "\n").replace("'", "")
                    chat_history.append((None, knowledge_new))
                    REQUIRED_INFO["current_stage"] == 'eda_generation'
        ...
        # Algorithm Selection
            ...
            if interactive_mode:
                chat_history.append('If you want to use a specific causal discovery algorithm, please enter it here! Type Quit to exit Interactive Mode', 
                    None)
                yield chat_history, download_btn
                if message == 'Quit':
                    interactive_mode = False
                REQUIRED_INFO["current_stage"] = 'validate_user_algo'
            else: 
                REQUIRED_INFO['current_stage'] == 'user_algo_selection'

            if REQUIRED_INFO['current_stage'] == 'validate_user_algo':
                if user_algo_selection(message) == 'Invalid':
                    chat_history.append('That algorithm may not be correct, please enter new data here!', None)
                    return chat_history, download_btn
                else:
...
if __name__ == "__main__":
    ...
    demo.launch(share=True)import gradio as gr
import pandas as pd
import io
import os
import shutil
